...
function gommero_optionsframework_add_admin() {
    global $query_string;

    $themename = gommero_get_option('of_themename');
    $shortname = gommero_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = gommero_get_option('of_template');
            gommero_reset_options($options, 'optionsframework');
            header("Location: admin.php?page=optionsframework&reset=true");
            die;
        }
    }
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'gommero_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'gommero_load_only');
}

add_action('admin_menu', 'gommero_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */

function gommero_reset_options($options, $page = '') {
    global $wpdb;
    $count = 0;
    ...
    if ($page == 'optionsframework') {
        gommero_delete_option('of_options');
    }
}
...
function gommero_optionsframework_options_page() {
    $options = gommero_get_option('of_template');
    $themename = gommero_get_option('of_themename');
    ?>
    <div class="trail-notify">
        <h1>Upgrade to Gommero Theme PRO!</h1>
        <p>You are Using the Lite Version of Gommero Theme. Upgrade to Pro for extra features like Home Page Slider, Contact Page Template, Gallery Page Template, FullWidth Page Template, Multiple Color Options and much more.</p>
        <a href="http://www.inkthemes.com/wp-themes/gommero-theme/" target="blank">Upgrade to Gommero Theme PRO here</a>
    </div>
    ...
    $return = gommero_optionsframework_machine($options);
    ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = gommero_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = gommero_get_option($option_id);
                }
                ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - gommero_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'gommero_ajax_callback');

function gommero_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        gommero_update_option($clickedID, $uploaded_file['url']);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = gommero_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            gommero_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    gommero_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    gommero_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            gommero_update_option($multicheck_id, 'false');
                        } else {
                            gommero_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    gommero_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    gommero_update_option($id, $border_array);
...
function gommero_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = gommero_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'select':
                ...
                $select_value = gommero_get_option($value['id']);
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= gommero_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= gommero_optionsframework_uploader_function($value['id'], $value['std'], 'min');

                break;
            case "color":
                $val = $value['std'];
                $stored = gommero_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = gommero_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = gommero_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                ...
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = gommero_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - gommero_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */

function gommero_optionsframework_uploader_function($id, $std, $mod) {
    ...
    $uploader = '';
    $upload = gommero_get_option($id);

    if ($mod != 'min') {
        $val = $std;
        if (gommero_get_option($id) != "") {
            $val = gommero_get_option($id);
        }
